...
class Admintuk_model extends CI_Model 
{
	public function __construct()
	{
  	...
  	function get_admintuk_by_id($user_id, $select)
  	{
	    try
	    {
		    $query = $this->db->select($select)
		    	->from('v_user_admin_tuk')
  	...
  	function get_admintuk_list($graph)
  	{
	    try
	    {
		    // select
		    ...
		    log_message("error", 'Error get_admintuk_list: '.json_encode($graph)."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function get_admintuk_count($graph)
  	{
	    try
	    {
		    // select
		    ...
		    log_message("error", 'Error get_admintuk_count: '.json_encode($graph)."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function create_admintuk($params, $auto_commit = TRUE)
  	{
		try 
		{
			if ($auto_commit) $this->db->trans_start();
			$admintuk_id = $this->db->insert('tbl_admin_tuk', $params);
		}
		catch (PDOException $e) {
			log_message("error", 'Error create_admintuk: '.json_encode($params)."<||>" . $e->getMessage());
  			$this->db->trans_rollback();
  			throw new PDOException($e->getMessage(), 1);
		}
		
		$admintuk_id = $this->db->insert_id();
		if ($auto_commit) $this->db->trans_complete();
		return $admintuk_id;
  	}

  	function update_admintuk_by_id($params, $user_id, $auto_commit = TRUE)
	{
		$affeted_rows = 0;
		...
		try 
		{
			...
			$affeted_rows = $this->db->update('tbl_admin_tuk');
			if ($auto_commit) $this->db->trans_complete();
		}
		catch (PDOException $e) {
			log_message("error", 'Error update_admintuk_by_id: '.$user_id."<||>" . $e->getMessage());
  			$status = FALSE;
  			throw new PDOException($e->getMessage(), 1);
		}
	...
	function delete_admintuk_by_user_id($admintuks, $auto_commit = TRUE)
	{
		$affeted_rows = 0;
		if (empty($admintuks)) return TRUE;
		try 
		{
			if ($auto_commit) $this->db->trans_start();
			$affeted_rows = $this->db->update_batch('tbl_admin_tuk', $admintuks, 'user_id');
		}
		catch (PDOException $e) {
			log_message("error", 'Error delete_admintuk_by_user_id: '.json_encode($admintuks)."<||>" . $e->getMessage());
  			$affeted_rows = FALSE;
			if ($auto_commit) $this->db->trans_rollback();
  			throw new PDOException($e->getMessage(), 1);
		}

		if ($affeted_rows != count($admintuks))
		{
			if ($auto_commit) $this->db->trans_rollback();
		}
		else
