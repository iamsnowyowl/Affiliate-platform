...
class Applicant_model extends CI_Model 
{
	public function __construct()
	{
  	...
  	function get_applicant_by_id($user_id, $select)
  	{
	    try
	    {
  	...
  	function get_applicant_list($graph)
  	{
	    try
	    {
		    // select
		    ...
		    log_message("error", 'Error get_applicant_list: '.json_encode($graph)."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function get_applicant_count($graph)
  	{
	    try
	    {
		    // select
		    ...
		    log_message("error", 'Error get_applicant_count: '.json_encode($graph)."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function create_applicant($params, $auto_commit = TRUE)
  	{
		try 
		{
			if ($auto_commit) $this->db->trans_start();
			$applicant_id = $this->db->insert('tbl_applicant', $params);
		}
		catch (PDOException $e) {
			log_message("error", 'Error create_applicant: '.json_encode($params)."<||>" . $e->getMessage());
  	...
  	function update_applicant_by_id($params, $user_id, $auto_commit = TRUE)
	{
		$affeted_rows = 0;
		...
		try 
		{
			...
			$affeted_rows = $this->db->update('tbl_applicant');
			if ($auto_commit) $this->db->trans_complete();
		}
		catch (PDOException $e) {
			log_message("error", 'Error update_applicant_by_id: '.$user_id."<||>" . $e->getMessage());
  			$status = FALSE;
  			throw new PDOException($e->getMessage(), 1);
		}
	...
	function delete_applicant_by_user_id($applicants, $auto_commit = TRUE)
	{
		$affeted_rows = 0;
		if (empty($applicants)) return TRUE;
		try 
		{
			if ($auto_commit) $this->db->trans_start();
			$affeted_rows = $this->db->update_batch('tbl_applicant', $applicants, 'user_id');
		}
		catch (PDOException $e) {
			log_message("error", 'Error delete_applicant_by_user_id: '.json_encode($applicants)."<||>" . $e->getMessage());
  			$affeted_rows = FALSE;
			if ($auto_commit) $this->db->trans_rollback();
  			throw new PDOException($e->getMessage(), 1);
		}

		if ($affeted_rows != count($applicants))
		{
			if ($auto_commit) $this->db->trans_rollback();
		}
		else
