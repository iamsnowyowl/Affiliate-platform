...
class Flow_model extends CI_Model 
{
	...
	function check($check, $graph)
  	{
	    try
	    {
		    $this->db->select($graph->select)->from('v_flow');
  	...
  	function get_flow_by_id($flow_id, $graph)
  	{
	    try
	    {
		    $this->db->select($graph->select)->from('v_flow');
		    ...
		    log_message("error", 'Error get_flow_by_id: '.$flow_id."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function get_flow_list($graph)
  	{
	    try
	    {
		    ...
		    log_message("error", 'Error get_flow_list: '.json_encode($graph)."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function get_flow_count($graph)
  	{
	    try
	    {
		    ...
		    log_message("error", 'Error get_flow_count: '.json_encode($graph)."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function create_flow($params, $auto_commit = TRUE)
  	{
		try 
		{
			if ($auto_commit) $this->db->trans_start();
			$flow_id = $this->db->insert('tbl_flow', $params);
		}
		catch (PDOException $e) {
			log_message("error", 'Error create_flow: '.json_encode($params)."<||>" . $e->getMessage());
  	...
  	function update_flow_by_id($params, $flow_id, $auto_commit = TRUE)
	{
		$affeted_rows = 0;
		...
		try 
		{
			...
			$affeted_rows = $this->db->update('tbl_flow');
			if ($auto_commit) $this->db->trans_complete();
		}
		catch (PDOException $e) {
			log_message("error", 'Error update_flow_by_id: '.$flow_id."<||>" . $e->getMessage());
  			$status = FALSE;
  			throw new PDOException($e->getMessage(), 1);
		}
	...
	function delete_soft_flow_by_id($flows, $auto_commit = TRUE)
	{
		$affeted_rows = 0;
		...
		{
			if ($auto_commit) $this->db->trans_start();
			$affeted_rows = $this->db->update_batch('tbl_flow', $flows, 'flow_id');
		}
		catch (PDOException $e) {
			log_message("error", 'Error delete_soft_flow_by_id: '.json_encode($flows)."<||>" . $e->getMessage());
  			$affeted_rows = FALSE;
			if ($auto_commit) $this->db->trans_rollback();
  			throw new PDOException($e->getMessage(), 1);
		}

		if ($affeted_rows != count($flows))
		{
			if ($auto_commit) $this->db->trans_rollback();
		}
		else
	...
	function delete_hard_flow_by_id($flow_id, $auto_commit = TRUE)
	{
		$affeted_rows = 0;

		if (empty($flow_id)) return TRUE;
		try 
		{
			if ($auto_commit) $this->db->trans_start();
			
			$tables = array('tbl_flow');
			...
			log_message("error", 'Error delete_hard_flow_by_id: '.json_encode($flow_id)."<||>" . $e->getMessage());
  			$affeted_rows = FALSE;
			if ($auto_commit) $this->db->trans_rollback();
  			throw new PDOException($e->getMessage(), 1);
		}
