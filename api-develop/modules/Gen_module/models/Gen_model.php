...
class Gen_model extends CI_Model 
{
	...
	function check($check, $graph)
  	{
	    try
	    {
		    $this->db->select($graph->select)->from('v_gen');
		    $this->db->where(array('gen_name' => $check));
  	...
  	function get_gen_by_id($gen_id, $graph)
  	{
	    try
	    {
		    $this->db->select($graph->select)->from('v_gen');
		    $this->db->where(array('gen_id' => $gen_id));
		    ...
		    log_message("error", 'Error get_gen_by_id: '.$gen_id."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function get_gen_list($graph)
  	{
	    try
	    {
		    ...
		    log_message("error", 'Error get_gen_list: '.json_encode($graph)."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function get_gen_count($graph)
  	{
	    try
	    {
		    ...
		    log_message("error", 'Error get_gen_count: '.json_encode($graph)."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function create_gen($params, $auto_commit = TRUE)
  	{
		try 
		{
			if ($auto_commit) $this->db->trans_start();
			$gen_id = $this->db->insert('tbl_gen', $params);
		}
		catch (PDOException $e) {
			log_message("error", 'Error create_gen: '.json_encode($params)."<||>" . $e->getMessage());
  	...
  	function update_gen_by_id($params, $gen_id, $auto_commit = TRUE)
	{
		$affeted_rows = 0;
		...
		try 
		{
			...
			$affeted_rows = $this->db->update('tbl_gen');
			if ($auto_commit) $this->db->trans_complete();
		}
		catch (PDOException $e) {
			log_message("error", 'Error update_gen_by_id: '.$gen_id."<||>" . $e->getMessage());
  			$status = FALSE;
  			throw new PDOException($e->getMessage(), 1);
		}
	...
	function delete_soft_gen_by_id($gens, $auto_commit = TRUE)
	{
		$affeted_rows = 0;
		if (empty($gens)) return TRUE;

		try 
		{
			if ($auto_commit) $this->db->trans_start();
			$affeted_rows = $this->db->update_batch('tbl_gen', $gens, 'gen_id');
		}
		catch (PDOException $e) {
			log_message("error", 'Error delete_soft_gen_by_id: '.json_encode($gens)."<||>" . $e->getMessage());
  			$affeted_rows = FALSE;
			if ($auto_commit) $this->db->trans_rollback();
  			throw new PDOException($e->getMessage(), 1);
		}

		if ($affeted_rows != count($gens))
		{
			if ($auto_commit) $this->db->trans_rollback();
		}
		else
	...
	function delete_hard_gen_by_id($gen_id, $auto_commit = TRUE)
	{
		$affeted_rows = 0;

		if (empty($gen_id)) return TRUE;
		try 
		{
			if ($auto_commit) $this->db->trans_start();
			
			$tables = array('tbl_gen');
			...
			log_message("error", 'Error delete_hard_gen_by_id: '.json_encode($gen_id)."<||>" . $e->getMessage());
  			$affeted_rows = FALSE;
			if ($auto_commit) $this->db->trans_rollback();
  			throw new PDOException($e->getMessage(), 1);
		}
