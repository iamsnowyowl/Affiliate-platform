...
class Kuk_model extends CI_Model 
{
	...
	function check($check, $graph)
  	{
	    try
	    {
		    $this->db->select($graph->select)->from('tbl_kuk');
  	...
  	function get_kuk_by_id($kuk_id, $graph)
  	{
	    try
	    {
		    $this->db->select($graph->select)->from('tbl_kuk');
		    $this->db->where(array('kuk_id' => $kuk_id));
		    ...
		    log_message("error", 'Error get_kuk_by_id: '.$kuk_id."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function get_kuk_list($graph)
  	{
	    try
	    {
		    ...
		    log_message("error", 'Error get_kuk_list: '.json_encode($graph)."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function get_kuk_count($graph)
  	{
	    try
	    {
		    ...
		    log_message("error", 'Error get_kuk_count: '.json_encode($graph)."<||>" . $e->getMessage());
	    	throw new PDOException($e->getMessage(), 1);
		}
  	...
  	function create_kuk($params, $auto_commit = TRUE)
  	{
		try 
		{
			if ($auto_commit) $this->db->trans_start();
			$kuk_id = $this->db->insert('tbl_kuk', $params);
		}
		catch (PDOException $e) {
			log_message("error", 'Error create_kuk: '.json_encode($params)."<||>" . $e->getMessage());
  			$this->db->trans_rollback();
  			throw new PDOException($e->getMessage(), 1);
		}
		
		$kuk_id = $this->db->insert_id();
		if ($auto_commit) $this->db->trans_complete();
		return $kuk_id;
  	}

  	function update_kuk_by_id($params, $kuk_id, $auto_commit = TRUE)
	{
		$affeted_rows = 0;
		...
		try 
		{
			...
			$affeted_rows = $this->db->update('tbl_kuk');
			if ($auto_commit) $this->db->trans_complete();
		}
		catch (PDOException $e) {
			log_message("error", 'Error update_kuk_by_id: '.$kuk_id."<||>" . $e->getMessage());
  			$status = FALSE;
  			throw new PDOException($e->getMessage(), 1);
		}
	...
	function delete_soft_kuk_by_id($kuks, $auto_commit = TRUE)
	{
		$affeted_rows = 0;
		if (empty($kuks)) return TRUE;

		try 
		{
			if ($auto_commit) $this->db->trans_start();
			$affeted_rows = $this->db->update_batch('tbl_kuk', $kuks, 'kuk_id');
		}
		catch (PDOException $e) {
			log_message("error", 'Error delete_soft_kuk_by_id: '.json_encode($kuks)."<||>" . $e->getMessage());
  			$affeted_rows = FALSE;
			if ($auto_commit) $this->db->trans_rollback();
  			throw new PDOException($e->getMessage(), 1);
		}

		if ($affeted_rows != count($kuks))
		{
			if ($auto_commit) $this->db->trans_rollback();
		}
		else
	...
	function delete_hard_kuk_by_id($kuk_id, $auto_commit = TRUE)
	{
		$affeted_rows = 0;

		if (empty($kuk_id)) return TRUE;
		try 
		{
			if ($auto_commit) $this->db->trans_start();
			
			$tables = array('tbl_kuk');
			...
			log_message("error", 'Error delete_hard_kuk_by_id: '.json_encode($kuk_id)."<||>" . $e->getMessage());
  			$affeted_rows = FALSE;
			if ($auto_commit) $this->db->trans_rollback();
  			throw new PDOException($e->getMessage(), 1);
		}
