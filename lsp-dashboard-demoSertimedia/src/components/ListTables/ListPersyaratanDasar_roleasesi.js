...
import { path_assessments, baseUrl, getData, getRole } from "../config/config";
...
class ListPersyaratanDasar_roleasesi extends Component<Props> {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  handleSearch = searchText => {
    const { IDAssessment } = this.props;
    this.setState({ loading: true });
    const auth = Digest("/me" + path_assessments + "/" + IDAssessment, "GET");
    reqwest({
      url:
        baseUrl +
        "/me" +
  ...
  get = (params = { type: "DASAR" }) => {
    const { IDAssessment } = this.props;
    this.setState({ loading: true });
    const auth = Digest(
      "/me" + path_assessments + "/" + IDAssessment + "/portfolios",
      "GET"
    );
    reqwest({
      url:
        baseUrl + "/me" + path_assessments + "/" + IDAssessment + "/portfolios",
      method: "GET",
      data: {
        limit: 10,
        ...params
  ...
  handleSubmitText = (event, errors, values) => {
    ...
    const { IDAssessment } = this.props;
    const path = "/me" + path_assessments + "/" + IDAssessment + "/portfolios";
    var data = {};
...
export default ListPersyaratanDasar_roleasesi;
