...
public class PersyaratanUmumNonAccountAdapter extends RecyclerView.Adapter<PersyaratanUmumNonAccountAdapter.PersyaratanUmumNonAccountHolder> {
    Context context;
    List<Portofolio> persyaratanUmum;
    long downloadID;
    Activity activity;

    public PersyaratanUmumNonAccountAdapter(Context context, Activity activity, List<Portofolio> portofolios, long downloadID) {
    ...
    @Override
    public PersyaratanUmumNonAccountHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_portofolio, parent, false);
        return new PersyaratanUmumNonAccountAdapter.PersyaratanUmumNonAccountHolder(itemView);
    }

    @Override
    public void onBindViewHolder(PersyaratanUmumNonAccountHolder holder, int position) {
        ...
        for (int i = 0; i < portofolio.getApplicantPortofolios().size(); i++) {
            if (portofolio.getApplicantPortofolios().size() > 1)
...
//        File file = new File(context.getExternalFilesDir(null), filename);
    ...
    public class PersyaratanUmumNonAccountHolder extends RecyclerView.ViewHolder {
        ...
        public PersyaratanUmumNonAccountHolder(View itemView) {
            super(itemView);
            ButterKnife.bind(this, itemView);

            for (int index = 0; index < persyaratanUmum.size(); index++) {
                int indexChild = persyaratanUmum.get(index).getApplicantPortofolios().size();
